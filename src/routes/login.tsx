...
function LoginPage() {
  ...
  useEffect(() => {
    fetch("/api/auth", { credentials: "include" }).then(response => {
    ...
    <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl place-items-center">
      <div className="grid w-full overflow-hidden rounded-[28px] border border-[#dfe8e1] bg-white shadow-[0_24px_80px_rgba(24,64,35,.10)] md:grid-cols-2">
        <section className="hidden bg-[#123d25] p-10 text-white md:flex md:flex-col md:justify-between">
          ...
          <div><span className="mb-4 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs">Operação protegida</span><h1 className="text-3xl font-semibold leading-tight">Suas sessões, campanhas e filas em um único lugar.</h1><p className="mt-4 text-sm leading-6 text-white/65">O acesso usa autenticação Supabase e cookies HttpOnly. Credenciais do gateway nunca são enviadas ao navegador.</p></div>
          <div className="flex items-center gap-2 text-xs text-white/60"><ShieldCheck size={16}/>Isolamento por organização e permissões Owner/Admin</div>
        </section>
        <section className="p-7 sm:p-10">
          ...
          <span className="text-xs font-semibold uppercase tracking-[.16em] text-[#269451]">Acesso seguro</span><h2 className="mt-2 text-2xl font-semibold">Entrar no ZapFlow</h2><p className="mt-2 text-sm text-[#78877e]">Use o usuário vinculado à sua organização.</p>
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            <button disabled={loading} className="mt-2 h-12 w-full rounded-xl bg-[#249b4e] font-semibold text-white transition hover:bg-[#1f8844] disabled:cursor-wait disabled:opacity-60">{loading ? "Entrando..." : "Entrar"}</button>
          </form>
          <p className="mt-6 text-center text-xs leading-5 text-[#8b9890]">O ZapFlow não armazena sua senha. A validação é feita pelo Supabase Auth.</p>
        </section>
      </div>
    </div>
  </main>;
