...
import { createInstance, deleteInstance, getConnectionState, getQr, hasGatewayConfig, logoutInstance } from "../lib/gateway/evolution";
...
async function saveAccount(request: Request, organizationId: string, instanceName: string, status: string) {
  ...
  if (existing[0]) {
    const response = await fetch(`${url}/rest/v1/whatsapp_accounts?id=eq.${existing[0].id}`, { method: "PATCH", headers, body: JSON.stringify(payload) });
  ...
  const response = await fetch(`${url}/rest/v1/whatsapp_accounts`, { method: "POST", headers, body: JSON.stringify(payload) });
...
async function patchAccount(request: Request, organizationId: string, instanceName: string, patch: Record<string, unknown>) {
  const { url, headers } = getSupabaseConfig(request);
  const response = await fetch(`${url}/rest/v1/whatsapp_accounts?organization_id=eq.${encodeURIComponent(organizationId)}&session_id=eq.${encodeURIComponent(instanceName)}`, { method: "PATCH", headers, body: JSON.stringify({ ...patch, updated_at: new Date().toISOString() }) });
...
export const Route = createFileRoute("/api/gateway")({ server: { handlers: {
  GET: async ({ request }) => {
    try {
      ...
      requireAdmin(user);
      return Response.json({ ok: true, gatewayConfigured: hasGatewayConfig(), accounts: await listAccounts(request, organizationId) });
    } catch (error) {
  ...
  POST: async ({ request }) => {
    try {
      ...
      requireAdmin(user);
      if (!hasGatewayConfig()) return Response.json({ error: "Gateway Evolution ainda não configurado no servidor.", code: "GATEWAY_NOT_CONFIGURED" }, { status: 503 });
      ...
      switch (body.action) {
        case "create": {
          ...
          if (!exists && accounts.length >= 10) return Response.json({ error: "Limite de 10 sessões WhatsApp por organização atingido" }, { status: 409 });
          const origin = new URL(request.url).origin;
          const webhookSecret = runtimeEnv("GATEWAY_WEBHOOK_SECRET");
          result = await createInstance(safeName, webhookSecret ? `${origin}/api/gateway-webhook?secret=${encodeURIComponent(webhookSecret)}` : undefined);
          account = await saveAccount(request, body.organizationId, safeName, "CONNECTING");
          break;
        }
        case "qr":
          result = await getQr(safeName);
          await patchAccount(request, body.organizationId, safeName, { session_status: "WAITING_QR", connection_status: "WAITING_QR", qr_expires_at: new Date(Date.now() + 60_000).toISOString() });
          break;
        case "status": {
          result = await getConnectionState(safeName);
          const rawState = String(result.status || "").toUpperCase();
          const normalized = rawState.includes("OPEN") || rawState.includes("CONNECTED") ? "CONNECTED" : rawState.includes("CONNECTING") ? "CONNECTING" : "DISCONNECTED";
          await patchAccount(request, body.organizationId, safeName, { session_status: normalized, connection_status: normalized, reconnect_required: normalized === "DISCONNECTED", last_seen_at: new Date().toISOString(), connected_at: normalized === "CONNECTED" ? new Date().toISOString() : null });
          break;
        }
        case "logout":
          result = await logoutInstance(safeName);
          await patchAccount(request, body.organizationId, safeName, { session_status: "DISCONNECTED", connection_status: "DISCONNECTED", reconnect_required: true });
          break;
        case "delete": {
          result = await deleteInstance(safeName);
          const { url, headers } = getSupabaseConfig(request);
          await fetch(`${url}/rest/v1/whatsapp_accounts?organization_id=eq.${encodeURIComponent(body.organizationId)}&session_id=eq.${encodeURIComponent(safeName)}`, { method: "DELETE", headers });
          break;
        }
      }
