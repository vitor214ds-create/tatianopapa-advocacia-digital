  ...
  server: { handlers: {
    GET: async ({ request }) => {
      try {
        ...
        const user = await authorizeOrganization(request, organizationId); requireAdmin(user);
        return Response.json({ ok: true, accounts: await listAccounts(request, organizationId) });
      } catch (error) { if (error instanceof Response) return error; return Response.json({ error: error instanceof Error ? error.message : "Erro inesperado ao listar sessões" }, { status: 500 }); }
    },
    POST: async ({ request }) => {
      try {
        ...
        let result; let account: unknown = null;
        switch (body.action) {
          case "create": {
            ...
            account = await saveAccount(request, body.organizationId, safeName, "CONNECTING"); break;
          }
          ...
          case "status": {
            result = await getConnectionState(safeName); const rawState = String(result.status || "").toUpperCase();
            ...
            await patchAccount(request, body.organizationId, safeName, { session_status: normalized, connection_status: normalized, reconnect_required: normalized === "DISCONNECTED", last_seen_at: new Date().toISOString(), connected_at: normalized === "CONNECTED" ? new Date().toISOString() : null }); break;
          }
          ...
          case "delete": {
            result = await deleteInstance(safeName); const { url, headers } = getSupabaseConfig(request);
            await fetch(`${url}/rest/v1/whatsapp_accounts?organization_id=eq.${encodeURIComponent(body.organizationId)}&session_id=eq.${encodeURIComponent(safeName)}`, { method: "DELETE", headers }); break;
          }
        }
