    ...
    GET: async ({ request }) => {
      try {
        ...
        if (!profile && cookies[REFRESH_COOKIE]) {
          ...
          if (refreshed) { accessToken = refreshed.access_token; profile = await getProfile(accessToken); }
        }
        ...
        return response;
      } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Erro ao validar sessão" }, { status: 500 }); }
    },
    POST: async ({ request }) => {
      try {
        ...
        if (body.action !== "login" || !body.email || !body.password) return Response.json({ error: "E-mail e senha são obrigatórios" }, { status: 400 });
        const { url, anonKey } = supabaseConfig();
        const loginResponse = await fetch(`${url}/auth/v1/token?grant_type=password`, {
          method: "POST", headers: { apikey: anonKey, "Content-Type": "application/json" }, body: JSON.stringify({ email: body.email.trim(), password: body.password }),
        });
        if (!loginResponse.ok) return Response.json({ error: "E-mail ou senha inválidos" }, { status: 401 });
        ...
        return response;
      } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Erro inesperado no login" }, { status: 500 }); }
    },
  } },
});
