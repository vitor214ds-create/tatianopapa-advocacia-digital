...
import { FileText, Loader2, Pencil, Plus, RefreshCw, Save, Trash2, X } from "lucide-react";
...
export function TemplatesPage({ organizationId }: Props) {
  ...
  async function submit() {
    ...
    setSaving(true);
    try {
      await saveTemplate({ organizationId, ...(draft.id ? { id: draft.id } : {}), name, content, category: draft.category });
      setSuccess(draft.id ? "Template atualizado." : "Template criado.");
      setEditing(false);
      setDraft(emptyDraft);
      await refresh();
    } catch (err) {
  ...
  async function remove(template: MessageTemplate) {
    if (deleting || !window.confirm(`Excluir o template “${template.name}”?`)) return;
    setDeleting(template.id);
    setError(null);
    setSuccess(null);
    try {
      await deleteTemplate(organizationId, template.id);
      if (draft.id === template.id) { setEditing(false); setDraft(emptyDraft); }
  ...
  return <>
    <div className="page-heading">
      ...
      <div className="actions"><button className="btn btn-soft" onClick={() => void refresh()} disabled={loading}><RefreshCw size={16}/>Atualizar</button><button className="btn btn-primary" disabled={saving} onClick={startCreate}><Plus size={17}/>Criar template</button></div>
    </div>
    ...
    {editing && <section className="panel mb-4">
      <div className="panel-title"><div><span className="eyebrow">{draft.id ? "Editar" : "Novo"}</span><h2>{draft.id ? "Editar template" : "Criar template"}</h2></div><button className="btn btn-soft" disabled={saving} onClick={() => setEditing(false)}><X size={16}/>Cancelar</button></div>
      ...
      {loading ? <div className="grid min-h-52 place-items-center text-sm text-[#718077]"><div className="text-center"><Loader2 size={24} className="mx-auto mb-2 animate-spin"/>Carregando templates...</div></div> : templates.length === 0 ? <div className="empty-state"><div className="empty-icon"><FileText size={23}/></div><h3>Nenhum template criado</h3><p>Crie seu primeiro template para reutilizar mensagens nas campanhas.</p><button className="btn btn-primary" onClick={startCreate}><Plus size={16}/>Criar template</button></div> : <div className="grid gap-3 p-4">
        {templates.map(template => <article key={template.id} className="rounded-2xl border border-[#e2e9e3] bg-white p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex gap-2"><button className="btn btn-soft" disabled={saving || Boolean(deleting)} onClick={() => startEdit(template)}><Pencil size={15}/>Editar</button><button className="btn btn-soft" disabled={saving || Boolean(deleting)} onClick={() => void remove(template)} title="Excluir"><Trash2 size={15}/></button></div>
          </div>
